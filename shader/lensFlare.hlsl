...
void mainComputeScaleShiftColor(uint3 dispatchID : SV_DispatchThreadID)
{
	...
	if (index == GHOSTCOUNT)
	{//バースト
		float2 scalingWeight = 1 / length(pos - center + 1) + computeConstants.r * computeConstants.r + 1;
		scalingWeight.x *= 2 * computeConstants.screenHeight / computeConstants.screenWidth;

		RWscaleShiftTbl[index].xy = scalingWeight * 0.3f;
	...
	{//ゴースト
		...
		float2 scalingWeight = ((computeConstants.r).xx + 1.xx) * weight * (lengthCenter + 1);
		scalingWeight.x *= 2 * computeConstants.screenHeight / computeConstants.screenWidth;
