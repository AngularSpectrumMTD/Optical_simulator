...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow)
{
  ...
  wc.lpfnWndProc = WndProc;//コールバック関数の設定
  ...
  //try
  {
    theApp.Initialize(hwnd, DXGI_FORMAT_R8G8B8A8_UNORM, false);//ここでComputeFilterAppの親のD3D12AppBaseのInitializeが呼ばれる

    SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(&theApp));
    ShowWindow(hwnd, nCmdShow);//表示の際にコールバックを呼ぶ
